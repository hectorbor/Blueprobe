...
def format_packets(values_timed,protocols_list):
    res=array.array
    for i in values_timed:
        current_protocol=i['protocol']
        match current_protocol:
            case 'HCI_CMD':
                {"order":i[1],"time":i[2], "OGF":i[2],"OCF":i[2],"Bytes":i[2]}
            case 'HCI_EVT':
                {"order":i[1],"time":i[2], "Code":i[1], "Command_Opcode":i[1]}
            case 'L2CAP':
                {"order":i[0],"time":i[1], "source":i[2],"destination":i[3]}
            case 'RFCOMM':
                {"order":i[0],"time":i[1], "source":i[2],"destination":i[3]}
            case 'AVCTP':
                {"order":i[0],"time":i[1], "source":i[2],"destination":i[3]}
